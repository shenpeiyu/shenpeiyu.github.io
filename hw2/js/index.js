...
$(document).ready(function() {
        ...
        var newItem = (item) => {
            ...
            $info = $('<div>').attr('class', 'info').append($h3).append($p)
            $col = $('<div>').attr('class', 'col-*').append($item).append($info)
        ...
        $('#pills-query-tab').on('click', function() {
            $.get('https://js.kchen.club/B07106012/query', function(response) {
                if (response) {
                    // 伺服器有回傳資料
                    if (response.result) {
                        $('#product-list').empty();
